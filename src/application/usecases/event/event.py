...
class AddEventsUseCase(BaseUseCase):
    async def execute(self):
        ...
        async for batch in EventsPaginator(self.client, start_date):
            ...
            batch_max = max(
                datetime.datetime.fromisoformat(item["changed_at"]) for item in batch
            )
            ...
            async with self.uow:
                await self.uow.places_repo.upsert_all(list(places.values()))
                await self.uow.events_repo.upsert_all(events)
        ...
        async with self.uow:
            ...
            if db_meta:
                ...
                db_meta.last_sync_time = datetime.datetime.now(datetime.timezone.utc)
